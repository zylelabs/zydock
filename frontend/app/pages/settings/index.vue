<script setup lang="ts">
  ...
  type TabId = 'general' | 'team' | 'git' | 'templates' | 'updates' | 'migration' | 'danger';
  ...
  const TABS = computed<{ id: TabId; label: string }[]>(() => [
    ...
    ...(isSuperuser.value ? [{ id: 'migration' as const, label: 'Migration' }] : []),
...
<template>
  <Content>
    ...
    <div v-else class="flex max-w-215 flex-col gap-4.5">
      ...
      <MigrationTab v-else-if="activeTab === 'migration' && isSuperuser" />
